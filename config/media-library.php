...
return [
     ...
     *
     * Read from config/cms.php so the media disk has exactly one source of
     * truth, and is not settable per-environment via a MEDIA_DISK env var. A
     * site operator who sets MEDIA_DISK=s3 in a .env file would otherwise
     * silently route uploads off local disk and break the rule without
     * touching a line of committed code.
     *
     * tests/Architecture/NoCloudStorageTest.php asserts this resolves to
     * 'public'. Requirements 2.3, 2.4.
     */
    'disk_name' => config('cms.media.disk', 'public'),

    /*
     * Conversions live on the same local disk as their originals. The upstream
     * default of null already means "same disk"; it is pinned explicitly here
     * because the upstream comment suggests splitting originals onto remote
     * storage, which this project forbids.
     */
    'conversions_disk_name' => config('cms.media.disk', 'public'),
